...
export async function main(ns) {
	...
	if (opts.help === true) {
		...
		typeof opts.blur == "boolean" ||
		opts.blur < 0 ||
		opts.blur > 10 ||
		!Number.isInteger(Math.floor(opts.blur))
	) {
		error(ns, "blur must be an integer from 0 to 10.");
	} else if (
		typeof opts.color == "boolean" ||
		!Number.isInteger(Math.floor(opts.color))
	) {
		// We don't actually validate which integer (see later)
		error(ns, "color must be an integer from 0 to 359.");
	} else if (
		typeof opts.delay == "boolean" ||
		opts.delay < 1 ||
		opts.delay > 10000 ||
		!Number.isInteger(Math.floor(opts.delay))
	) {
		...
		(opts.rainbow < 1 ||
			opts.rainbow > 100 ||
			!Number.isInteger(Math.floor(opts.rainbow)))
	) {
		error(ns, "rainbow must be an integer from 1 to 100.");
	} else if (
		typeof opts.opacity == "boolean" ||
		opts.opacity < 0 ||
		opts.opacity > 100 ||
		!Number.isInteger(Math.floor(opts.opacity))
	) {
	...
	ns.print(
		sprintf(
			"INFO:  Delay: %dms  Hue: %d°  Opacity: %d%%",
			Math.floor(opts.delay),
			Math.floor(opts.color),
			Math.floor(opts.opacity),
		),
	);
	ns.print(
		sprintf(
			"INFO:  Blur: %dpx   Rainbow: %s",
			Math.floor(opts.blur),
			opts.rainbow === false ? "Off" : Math.floor(opts.rainbow) + "s",
		),
	);
...
function matrixCSS(opts) {
	let anim = sprintf(
		" rainbow %ds infinite",
		opts.rainbow !== false ? opts.rainbow : 0,
	);
	return [
		...
		"  filter:" +
			sprintf(" hue-rotate(%ddeg)", Math.floor(opts.color) % 360) + // mod color degrees to 0-359
			/*         */ sprintf(" blur(%dpx)", Math.floor(opts.blur)) +
			/*         */ ";",
		opts.rainbow !== false ? "  -webkit-animation:" + anim + ";" : "",
		"}",
		"",
